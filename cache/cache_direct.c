...
ReadMem(int Addr)
{
	...
	tag = (Addr & 0xFFFC0000) >> 18;
	index = (Addr & 0x0003FFC0) >> 6;
	offset = (Addr & 0x0000003F);
	dest = (Addr & 0xFFFFFFC0);
	temp = (Cache[index].tag << 18) | (index << 6);
	...
	else
	{
		if(Cache[index].dirty)
		{
			for(i=0;i<16;i++)
			{
				M[temp/4 + i] = Cache[index].data[i];
			}
			for(i=0;i<16;i++)
			{
				Cache[index].data[i] = M[dest/4 + i];
			}
			Cache[index].dirty = 0;
			conflict_miss++;
		}
